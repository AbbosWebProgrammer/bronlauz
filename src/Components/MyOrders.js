...
const MyOrders = (props) =>{
    ...
    useEffect(() => {
        axios.get(API_PATH+"OrderJson/",headers).then((res) => {
    ...
    function addorremovemyfavorite(id) { 
        ...
        .then((res) => {
            console.log('id',id);
            let updatedList = allcountryhouses.map(item => 
                {
                  if (item.id == id){
            ...
            setallcountryhouses(updatedList);
            let updatedList1 = countryhouses.map(item => 
                {
                  if (item.id == id){
                    return {...item, heart: !item.heart}; 
                  }
    ...
    return (<>
      <YMInitializer  accounts={[90594448]} options={{webvisor: true}} version="2"/>
            ...
            :
            <div className='webcontainer'>  
                {allcountryhouses.length===0?
                <div className='row_css_home_page  min-vh-75 p-3 mb-5'>
                    <h1>Поездки</h1>
                    <hr/>
                    <h4>Пока ничего не запланировано </h4>
                                        ...
                                        <Col className='mb-4'  xs={12} sm={6} md={4} lg={3} >
                                            <div className="countryhousemenu ">
                                                ...
                                                <Link to={"/countryhouse/"+`${item.id}`} className="countryhouse_title_part">
                                                    ...
                                                    <Row className='mt-3'>
                                                        <Col sm={3} xs={3} md={3} lg={3} className="text-start">
                                                            <h4 className='size text-black'>Заезда:</h4>
                                                        </Col>
                                                        <Col sm={9} xs={9} md={9} lg={9} >
                                                            <p className='size text-black text-end'> {item.order_bron_days[0]} {item.start_time}</p>
                                                        </Col>
                                                    </Row>
                                                    <Row>
                                                        <Col sm={3} xs={3} md={3} lg={3} className="text-start">
                                                            <h4 className='size text-black'>Выезда:</h4>
                                                        </Col>
                                                        <Col sm={9} xs={9} md={9} lg={9} >
                                                            <p className='size text-black text-end'> {item.order_bron_days[item.order_bron_days.length-1]} {item.finish_time}</p>
                                                        </Col>
                                                    </Row>
                                                    <Row>
                                                        <Col sm={3} xs={3} md={3} lg={3} className="text-start">
                                                            <h3 className='size text-black'>Итого:</h3>
                                                        </Col>
                                                        <Col sm={9} xs={9} md={9} lg={9} >
                                                            <h3 className='size text-black text-end'>{item.allsumm} сум </h3>
                                                        </Col>
                                                    </Row>
                                            
                                                    {item.status===true?
                                                    <button className='button order-status-button bg-success text-white'>Подтверждено ✅</button>
                                                    :""
                                                    }
                                                    {item.status===null?
                                                    <button className='button order-status-button bg-primary text-white'>⏳ Проверяется… </button>

                                                    :""
                                                    }
                                                    {item.status===false?
                                                    <button className='button order-status-button bg-danger text-white'>Отказано ❌</button>
                                                    :""
                                                    }
                                                </Link>
                                            </div>
